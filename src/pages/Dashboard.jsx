...
export default function Dashboard({ session }) {
  ...
  return (
    <div style={{ padding: '15px', maxWidth: '500px', margin: '0 auto', fontFamily: 'sans-serif', backgroundColor: '#f8fafc', minHeight: '100vh', color: '#1e293b' }}>
      <header style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
          <h2 style={{ margin: 0, fontSize: '1.2em' }}>{todayKcal} / {profile.daily_goal_kcal} kcal</h2>
      ...
      <section style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          ...
          <select value={profile.activity} onChange={e => setProfile({...profile, activity: e.target.value})} style={{...inputStyle, gridColumn: 'span 2'}}>
            <option value="1.2">Brak ruchu (1.2)</option><option value="1.375">Niska aktywność (1.3)</option><option value="1.5">Średnia aktywność (1.5)</option><option value="1.9">Duża (1.9)</option>
          </select>
          ...
          <button onClick={saveAll} disabled={loading} style={btnStyle}>{loading ? '...' : 'Aktualizuj Plan (AI)'}</button>
        </div>
      </section>
      ...
      <div style={{ marginTop: '20px' }}>
        {meals.map(meal => (
          <div key={meal.id} style={mealItemStyle}>
            <div style={{ flex: 1 }}>
              <strong style={{ display: 'block', color: '#1e293b' }}>{meal.name}</strong>
      ...
      <button onClick={() => supabase.auth.signOut()} style={{...logoutStyle, color: '#94a3b8'}}>Wyloguj</button>
    </div>
  );
}

const cardStyle = { backgroundColor: '#fff', padding: '15px', borderRadius: '15px', marginBottom: '15px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' };
const inputStyle = { padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0', width: '100%', boxSizing: 'border-box', backgroundColor: '#ffffff', color: '#1e293b', fontSize: '16px', WebkitAppearance: 'none' };
const btnStyle = { gridColumn: 'span 2', padding: '14px', backgroundColor: '#1e293b', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', fontSize: '15px' };
const progressBg = { width: '100%', height: '10px', background: '#e2e8f0', borderRadius: '10px', overflow: 'hidden' };
const progressFill = { height: '100%', transition: 'width 0.5s' };
const dateStyle = { border: 'none', background: '#f1f5f9', padding: '8px', borderRadius: '8px', fontSize: '0.85em', color: '#1e293b' };
...
const deleteBtnStyle = { background: '#fee2e2', color: '#ef4444', border: 'none', borderRadius: '50%', width: '30px', height: '30px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' };
const logoutStyle = { marginTop: '20px', width: '100%', background: 'none', border: 'none', padding: '10px', fontSize: '14px' };